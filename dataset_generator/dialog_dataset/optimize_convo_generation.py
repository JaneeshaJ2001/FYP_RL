"""
Multi-Turn Disaster Dialogue Generator  (async parallel edition)
====================================================================
...
Parallel pipeline
─────────────────
All episodes are fired concurrently via asyncio + AsyncOpenAI.
A semaphore caps simultaneous requests (default 20) so you stay within
OpenAI rate limits.  Each episode is still one independent API call —
quality is identical to the original sequential version, just much faster.

Typical speedup: ~10–15x on large runs (100+ episodes).
...
─────
    ...
    # 20 episodes, default concurrency (20 parallel calls)
    python generate_conversations.py --episodes 20

    # Tune down if you hit 429 rate-limit errors
    python generate_conversations.py --episodes 100 --concurrency 10
...
MODEL               = "gpt-4o-mini"
MAX_TOKENS          = 3500
DEFAULT_CONCURRENCY = 20    # max simultaneous API calls; tune down if hitting 429s
MAX_RETRIES         = 1     # number of retry attempts per failed episode
...
def validate_episode(ep: dict, expected_turns: int) -> tuple[bool, str]:
    ...
    for t in turns:
        ...
        if rr:
            ...
            # ── Fix: reject turns that re-cite already-retrieved chunks ──────
            already_seen = set(chunks) & seen_chunk_ids
            new_chunks   = set(chunks) - seen_chunk_ids

            if not new_chunks:
                return False, (
                    f"turn {tid} retrieval_required=true but all cited chunks "
                    f"{set(chunks)} were already retrieved in earlier turns"
                )
            if already_seen:
                return False, (
                    f"turn {tid} relevant_chunks contains already-retrieved chunks "
                    f"{already_seen} — only list newly retrieved chunk IDs here"
                )
            # ─────────────────────────────────────────────────────────────────
...
def _apply_meta(ep: dict, plan: dict, tmpl: dict) -> None:
    """Stamp episode-level metadata (setdefault — never overwrite model output)."""
    for k, v in [
...
async def call_gpt_async(
    ...
    """Single async API call, bounded by the shared semaphore."""
...
async def generate_episode(
    ...
    client: AsyncOpenAI,
    semaphore: asyncio.Semaphore,
    counter: dict,          # shared mutable dict for live progress printing
    total: int,
) -> dict | None:
    """
    Generate, validate, and (if needed) retry a single episode.
    Returns the episode dict on success, or None on failure.
    """
    sc    = plan["scenario"]
    tmpl  = get_scenario_template(sc)
    n_exp = tmpl["turns"]
    label = f"{plan['episode_id']} {sc}:{tmpl['name'][:22]}"

    prompt = build_prompt(plan)

    for attempt in range(1 + MAX_RETRIES):
        try:
            ep = await call_gpt_async(client, prompt, semaphore)
            ...
            counter["done"] += 1
            print(
                f"  [{counter['done']:>4}/{total}] {label}  "
                f"✗ API error: {type(exc).__name__}: {exc}"
            )
            return None

        _apply_meta(ep, plan, tmpl)
        ok, reason = validate_episode(ep, n_exp)

        if ok:
            counter["done"] += 1
            retry_tag = " (retry ok)" if attempt > 0 else ""
            print(f"  [{counter['done']:>4}/{total}] {label}  ✓{retry_tag}")
            return ep

        if attempt < MAX_RETRIES:
            # Will retry — don't print yet
            continue

        # Final failure
        counter["done"] += 1
        print(f"  [{counter['done']:>4}/{total}] {label}  ✗ skipped ({reason})")
        return None
...
async def run(
    ...
    counter   = {"done": 0}   # shared progress counter across tasks
    total     = len(episodes_plan)

    tasks = [
        generate_episode(plan, client, semaphore, counter, total)
        for plan in episodes_plan
    ]

    results = await asyncio.gather(*tasks)

    # Filter out None (failed episodes), preserve generation order
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Generate multi-turn disaster dialogue dataset (async parallel)"
    )
    p.add_argument("--input",  default=str(DEFAULT_INPUT_FILE), help="Path to chunks_grouped.json")
    p.add_argument("--output", default=str(DEFAULT_OUTPUT_FILE), help="Output conversations.json path")
    p.add_argument("--episodes",    type=int, default=DEFAULT_TOTAL, help="Total episodes to generate")
    p.add_argument("--concurrency", type=int, default=DEFAULT_CONCURRENCY,
                   help=f"Max parallel API calls (default {DEFAULT_CONCURRENCY}; lower if hitting 429s)")
...
def main():
    ...
    print(f"Scenario plan ({total_episodes} episodes, concurrency={args.concurrency}):")
    for sc, cnt in scenario_dist.items():
        tmpl = get_scenario_template(sc)
    ...
    print(f"\nGenerating {total_episodes} episodes in parallel...\n")
    t_start = time.time()

    all_episodes = asyncio.run(run(episodes_plan, api_key, args.concurrency))
    ...
    n_saved = meta["total_episodes"]
    failed  = total_episodes - n_saved
    per_ep  = f"  (~{elapsed / n_saved:.1f}s/ep)" if n_saved else ""

    print(f"\n{'=' * 68}")
    print(
        f"Done!  saved={n_saved}  failed={failed}  "
        f"time={elapsed:.1f}s{per_ep}"
    )
